...
function averageAge(num1, num2, num3){
    if(typeof(num3) != Number)
        ...
        return Math.round((Number(num1)+Number(num2)+Number(num3))/3);
}
...
function leetSpeak(word){
    ...
    for(let i = 0; i < word.length; i++){
        if(newWord.charAt(i) == 'e') newWord = newWord.replace('e', '3');
        if(newWord.charAt(i) == 'a') newWord = newWord.replace('a', '4');
    }
    return newWord;
}

console.log(helloWorld());
console.log(greeting(prompt("Enter a name")));
console.log(divisibleByThree(prompt("Enter a number")));
console.log(averageAge(prompt("Enter first number"), prompt("Enter second number"), prompt("Enter third number")));
console.log(leetSpeak(prompt("Enter a word")));